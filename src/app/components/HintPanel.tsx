  ...
  return (
    <aside className="bg-[var(--panel)] backdrop-blur-[12px] border border-[var(--line)] rounded-[18px] overflow-y-auto flex flex-col shadow-[0_12px_40px_rgba(0,0,0,0.35)] w-[332px] shrink-0">
      ...
      <div className="p-[14px] flex flex-col gap-[13px]">
        {/* Persona Card */}
        <div className="border border-[var(--line)] rounded-[14px] bg-[var(--panel2)] p-[14px] flex gap-[13px] items-center">
        ...
        <div className="border border-[var(--line)] rounded-[14px] bg-[var(--panel2)] p-[14px] flex flex-col gap-[9px]">
          {levelData.hints.map((hint: string, i: number) => {
            const isRevealed = i < hintsRevealed;
            const cost = HINT_COSTS[i] || 10;
            
            return (
              <div key={i} className="border border-[var(--line)] rounded-[12px] overflow-hidden bg-[var(--bg)] transition duration-200">
                {isRevealed ? (
                  <div className="p-[12px_14px] text-[12.5px] leading-[1.6] text-[var(--cyan)] font-mono border-l-[3px] border-[var(--cyan)] bg-transparent animate-[msgIn_0.35s]">
                    <span className="block text-[9.5px] tracking-[2px] text-[var(--cyan)] mb-[5px]">
                      HINT {i + 1} · UNLOCKED
                    </span>
                    {hint}
                  </div>
                  ...
                  <button 
                    ...
                    disabled={score < cost}
                    className="w-full flex items-center gap-[10px] p-[12px_14px] cursor-pointer bg-transparent border-none text-[var(--text)] font-sans text-[12.5px] text-left transition duration-200 hover:bg-[var(--panel2)] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <span className="text-[15px]">🔒</span>
                    Reveal Hint {i + 1}
                    <span className="ml-auto font-mono text-[10.5px] text-[var(--cyan)] border border-[var(--line)] rounded-[6px] p-[2px_8px] whitespace-nowrap">
                      −{cost} pts
                    </span>
                  </button>
                )}
        ...
        <div className="grid grid-cols-2 gap-[9px]">
          <div className="border border-[var(--line)] rounded-[11px] p-[10px_12px] bg-[var(--bg)]">
